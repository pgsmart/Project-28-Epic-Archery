...
function draw() {
  ...
  for (var i = 0; i < playerArrows.length; i++) {
      if (playerArrows[i] !== undefined && boards[a] !== undefined) {
          collision = Matter.SAT.collides(playerArrows[i].body,boards[a].body);
          if(collision.collided){
            console.log("HIT");
            hits++;
            Matter.World.remove(world, boards[a].body);
            delete boards[a];
          }
      }
